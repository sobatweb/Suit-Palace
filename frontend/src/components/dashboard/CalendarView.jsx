        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-7 border rounded-2xl overflow-hidden shadow-inner bg-white">
            {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map(d => (
              <div key={d} className="bg-gray-50 py-3 text-center text-[9px] font-black uppercase text-gray-400 border-b">{d}</div>
            ))}
              ...
              return (
                <div key={day} onClick={() => setSelectedDay(day)} className={`min-h-[95px] p-2 border-[0.5px] border-gray-50 cursor-pointer transition-all ${selectedDay === day ? 'bg-slate-100/50 ring-2 ring-inset ring-slate-200' : 'hover:bg-gray-50'}`}>
                  <div className="flex justify-between items-start mb-1">
                    <span className={`text-[10px] font-black ${selectedFullDate === dateStr ? 'bg-[#1A120B] text-white px-1.5 py-0.5 rounded' : 'text-gray-300'}`}>{day}</span>
          ...
          <div className="mt-6 pt-6 border-t border-gray-50 flex flex-wrap gap-5">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500"></div><span className="text-[9px] font-black uppercase text-gray-400">Overdue</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-yellow-400"></div><span className="text-[9px] font-black uppercase text-gray-400">Persiapan</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500"></div><span className="text-[9px] font-black uppercase text-gray-400">Booking</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500"></div><span className="text-[9px] font-black uppercase text-gray-400">Kembali</span></div>
          </div>
        </div>
